...
public class UserRestfulApiTest extends AbstractBaseControllerTest {
    ...
    public void delCustomerUser() {
        ...
        ResponseTemplate<Void> template2=getForObj(baseURL.toString() + "/delCustomerUser/{id}",
                new ParameterizedTypeReference<ResponseTemplate<Void>>() {
                },2);
    ...
    public void delCustomer() {
        ...
        Assert.assertEquals("请求错误，预期http状态码为200",200,template.getHttpCode());
        ResponseTemplate<Void> template2=getForObj(baseURL.toString() + "/delCustomer/{id}",
                new ParameterizedTypeReference<ResponseTemplate<Void>>() {
                },2);
        assertThat(template2.getHttpCode(),equalTo(200));
        assertThat(template2.getErrorMsg(),is("请先删除用户信息"));
    }
    ...
    public void addCustomer() {
        ...
        Assert.assertEquals("请求错误，预期http状态码为200",200,template.getHttpCode());
        //已存在机构下再插入一条数据
        ResponseTemplate<Void> template2 = getForObj(baseURL.toString() + "/addCustomer/{customerName}"
                , new ParameterizedTypeReference<ResponseTemplate<Void>>() {
                },"customer1");
        assertThat(template2.getHttpCode(),equalTo(500));
    ...
    public void checkPhone() {
        ...
        Assert.assertEquals("请求错误，预期http状态码为200",200,template.getHttpCode());
        //不存在插入一条数据
        ResponseTemplate<Void> template2=getForObj(baseURL.toString() + "/check/{phone}/{username}",
                new ParameterizedTypeReference<ResponseTemplate<Void>>() {
                },"13500002341","ws_user1");
        assertThat(template2.getHttpCode(),equalTo(400));
    }
