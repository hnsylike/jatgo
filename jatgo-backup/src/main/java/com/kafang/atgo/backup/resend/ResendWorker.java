...
public class ResendWorker {
		...
		if (resendConfig != null) {
			try {
				...
				UseLogger.error(log, e,
						"Call lambda QueueProcessor in taskQueue of resendConfig.getProcessName()==[{}] throw Exception -> {}",
						resendConfig.getProcessName(), e.getMessage());
	...
	public void resendMessage(FixResendRequest request) {
		if (taskQueue.enQueue(request)) {
			log.info("ProcessId -> ({}) Message resend request enQueue success.", request.getProcessId());
		} else {
			log.error("ProcessId -> ({}) Message resend request enQueue failure!", request.getProcessId());
		}
	}
