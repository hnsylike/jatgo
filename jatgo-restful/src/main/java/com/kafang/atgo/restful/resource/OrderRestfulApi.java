...
public class OrderRestfulApi extends AtgoRestfulApi {
	...
	@Consumes(JSON_UTF8)
	public ResponseTemplate<LogonResult> login(
			@ApiParam(name = "用户名", required = true) @PathParam("username") String username,
...
//                }
	...
	public ResponseTemplate<List<OrderResponse>> returnActual(
		...
		agClientService.getByUserId(userId).forEach(agClient -> {
		...
		List<OrderResponse> returnList = collectList.stream().filter(ord -> ord != null).map(p -> convertOrderInfo(p))
				.collect(Collectors.toList());
	...
	public ResponseTemplate<List<OrderResponse>> returnActualExecReportByClientId(
		...
		if (!execReports.isPresent()) {
			...
			return ResponseTemplate.failed(Response.Status.OK, "没有回报数据");
		}
		List<OrderResponse> rt = execReports.get().values().stream()
				.map(list -> list != null ? list.get(list.size() - 1) : null).map(p -> convertOrderInfo(p))
				.collect(Collectors.toList());
		if (rt == null) {
...
//        waService.insert(wat);
	...
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "根据userId查询回报rest api", notes = "rest api")
	@Consumes(JSON_UTF8)
	public ResponseTemplate<PageDto<OrderResponse>> queryOrderEightByUserId(
		...
		List<OrderResponse> returnList = collectList.stream().filter(ord -> ord != null && ord.getHandlInst() == inst)
				.map(p -> convertOrderInfo(p)).collect(Collectors.toList());

		PageDto<OrderResponse> pageDto = new PageDto<>(pageNo, pageSize, returnList.size(), returnList);
		log.info("REST: return Execution Reports of 8 by userId {}", pageDto.toString());
		return ResponseTemplate.ok(pageDto);
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "根据clientId查询回报rest api", notes = "rest api")
	@Consumes(JSON_UTF8)
	public ResponseTemplate<PageDto<OrderResponse>> queryOrderEightByClientId(
			...
			@ApiParam(required = true, name = "当前页") @PathParam("pageNo") Integer pageNo,
			@ApiParam(required = true, name = "每页条数") @PathParam("pageSize") Integer pageSize) throws Exception {
		log.info("REST: get Execution Reports of 8 by clientId {}", clientId);
		Optional<ConcurrentMap<Long, MutableList<FixOrder>>> execReports = rabbitMQService.getOrderReportMap(clientId);
		...
		if (execReports.isPresent()) {
			...
			final int inst = handlInst.intValue();
			returnList = rt.stream().filter(ord -> ord != null && ord.getHandlInst() == inst)
					.collect(Collectors.toList());
		}

		PageDto<OrderResponse> pageDto = new PageDto<>(pageNo, pageSize, returnList.size(), returnList);
		log.info("REST: return Execution Reports of 8 by clientId {}", pageDto.toString());
		return ResponseTemplate.ok(pageDto);
	}
	...
	private OrderResponse convertOrderInfo(FixOrder fixOrder) {
		...
		orderResponse.setOutstandingQty(fixOrder.getOutstandingQty());
